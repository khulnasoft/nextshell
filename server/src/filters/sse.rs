...
//! use nextshell::{Filter, sse::Event};
...
//! }
//!
//! let app = nextshell::path("push-notifications")
//!     .and(nextshell::get())
//!     .map(|| {
//!         nextshell::sse::reply(nextshell::sse::keep_alive().stream(sse_events()))
//!     });
//! ```
//!
...
///
/// ```
/// let app = nextshell::sse::last_event_id::<u32>();
///
...
///
/// async {
///     let app = nextshell::path("sse").and(nextshell::get()).map(|| {
...
/// use nextshell::{Filter, Stream, sse::Event};
///
...
///
/// fn main() {
///     let routes = nextshell::path("ticks")
///         .and(nextshell::get())
///         .map(|| {
...
///             let stream = nextshell::sse::keep_alive()
///                 .interval(Duration::from_secs(5))
///                 .text("thump".to_string())
///                 .stream(event_stream);
///             nextshell::sse::reply(stream)
///         });
/// }
/// ```
///
