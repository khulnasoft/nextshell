...
//! # Routing
//!
//! Routing in nextshell is simple yet powerful.
//!
...
//! let hello_from_nextshell = nextshell::path!("hello" / "from" / "nextshell").map(|| {
//!     "Hello from nextshell!"
//! });
//! ```
//!
...
//! # use nextshell::Filter;
//! // GET /sum/:u32/:u32
//! let sum = nextshell::path!("sum" / u32 / u32).map(|a, b| {
//!     format!("{} + {} = {}", a, b, a + b)
//! });
//! ```
//!
...
//! # use nextshell::Filter;
//! // GET /:u16/times/:u16
//! let times = nextshell::path!(u16 / "times" / u16).map(|a, b| {
...
//! ```
//! # use nextshell::Filter;
//! # let sum = nextshell::any().map(nextshell::reply);
//! # let times = sum.clone();
//! // GET /math/sum/:u32/:u32
//! // GET /math/:u16/times/:u16
//! let math = nextshell::path("math");
...
//! # use nextshell::Filter;
//! // GET /bye/:string
//! let bye = nextshell::path("bye")
//!     .and(nextshell::path::param())
//!     .map(|name: String| {
...
//! ```
//! # use nextshell::Filter;
//! # let sum = nextshell::path("sum");
//! # let times = nextshell::path("times");
//! // GET /math/sum/:u32/:u32
//! // GET /math/:u16/times/:u16
//! let math = nextshell::path("math")
//!     .and(sum.or(times));
...
//! ```
//! # use nextshell::Filter;
//! # let hi = nextshell::path("hi");
//! # let hello_from_nextshell = hi.clone();
//! # let bye = hi.clone();
//! # let math = hi.clone();
//! // GET /hi
//! // GET /hello/from/nextshell
...
///
/// ```
/// use nextshell::{Filter, path::FullPath};
/// use std::{collections::HashMap, sync::{Arc, Mutex}};
///
/// let counts = Arc::new(Mutex::new(HashMap::new()));
/// let access_counter = nextshell::path::full()
...
///
/// // Match `/sum/:a/:b`
/// let route = nextshell::path!("sum" / u32 / u32)
///     .map(|a, b| {
///         format!("{} + {} = {}", a, b, a + b)
///     });
/// ```
...
/// use nextshell::Filter;
///
/// let prefix = nextshell::path!("math" / "sum" / ..);
///
/// let sum = nextshell::path!(u32 / u32)
///     .map(|a, b| {
///         format!("{} + {} = {}", a, b, a + b)
///     });
///
/// let help = nextshell::path::end()
