...
/// let content_length = nextshell::header::<u64>("content-length");
///
/// // Parse `host: 127.0.0.1:8080` as a `SocketAddr
/// let local_host = nextshell::header::<SocketAddr>("host");
///
/// // Parse `foo: bar` into a `String`
/// let foo = nextshell::header::<String>("foo");
/// ```
...
/// let opt_auth = nextshell::header::optional::<String>("authorization");
/// ```
pub fn optional<T>(
    name: &'static str,
...
/// let must_dnt = nextshell::header::exact("dnt", "1");
/// ```
pub fn exact(
    name: &'static str,
    value: &'static str,
...
/// let keep_alive = nextshell::header::exact_ignore_case("connection", "keep-alive");
/// ```
...
///
/// ```
/// use nextshell::{Filter, http::header::HeaderValue};
///
/// let filter = nextshell::header::value("x-token")
...
///
/// ```
/// use nextshell::{Filter, http::HeaderMap};
///
/// let headers = nextshell::header::headers_cloned()
