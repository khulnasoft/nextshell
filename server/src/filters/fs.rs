...
/// let route = nextshell::fs::file("/www/static/app.js");
/// ```
...
/// let route = nextshell::path("static")
///     .and(nextshell::fs::dir("/www/static"));
///
/// // For example:
...
impl File {
    ...
    ///
    /// ```
    /// use nextshell::{Filter, reply::Reply};
    ///
    /// let route = nextshell::path("static")
    ///     .and(nextshell::fs::dir("/www/static"))
    ///     .map(|reply: nextshell::filters::fs::File| {
    ///         if reply.path().ends_with("video.mp4") {
    ///             nextshell::reply::with_header(reply, "Content-Type", "video/mp4").into_response()
    ///         } else {
    ///             reply.into_response()
    ///         }
    ///     });
