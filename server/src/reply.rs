...
//!
//! ```
//! use nextshell::{Filter, http::Response};
//!
//! // Returns an empty `200 OK` response.
//! let empty_200 = nextshell::any().map(nextshell::reply);
//!
//! // Returns a `200 OK` response with custom header and body.
//! let custom = nextshell::any().map(|| {
//!     Response::builder()
...
//! let routes = nextshell::get().and(empty_200)
//!     .or(nextshell::post().and(custom));
//! ```
...
/// let route = nextshell::path("just-ok")
///     .map(|| {
...
/// // `[1, 3, 7, 13]`
/// let route = nextshell::path("ids")
///     .map(|| {
///         let our_ids = vec![1, 3, 7, 13];
///         nextshell::reply::json(&our_ids)
///     });
/// ```
///
/// # Note
...
/// <html>
///     <head>
///         <title>HTML with nextshell!</title>
///     </head>
///     <body>
///         <h1>nextshell + HTML = &hearts;</h1>
///     </body>
/// </html>
/// "#;
///
...
/// }
///
/// impl nextshell::Reply for Message {
///     fn into_response(self) -> nextshell::reply::Response {
...
/// }
///
/// let route = nextshell::any().map(handler);
/// ```
...
///     .map(nextshell::reply)
///     .map(|reply| {
///         nextshell::reply::with_status(reply, nextshell::http::StatusCode::CREATED)
///     });
/// ```
...
/// Wrap an `impl Reply` to change its `StatusCode`.
///
/// Returned by `nextshell::reply::with_status`.
#[derive(Debug)]
...
///     .map(nextshell::reply)
///     .map(|reply| {
///         nextshell::reply::with_header(reply, "server", "nextshell")
///     });
/// ```
...
///
/// Returned by `nextshell::reply::with_header`.
#[derive(Debug)]
