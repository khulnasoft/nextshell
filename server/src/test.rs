...
//!
//! fn sum() -> impl Filter<Extract = (u32,), Error = nextshell::Rejection> + Copy {
//!     nextshell::path::param()
...
//! }
//!
//! fn math() -> impl Filter<Extract = (String,), Error = nextshell::Rejection> + Copy {
//!     nextshell::post()
//!         .and(sum())
//!         .map(|z: u32| {
//!             format!("Sum = {}", z)
...
//! #[tokio::test]
//! async fn test_sum() {
//! #    let sum = || nextshell::any().map(|| 3);
//!     let filter = sum();
//!
//!     // Execute `sum` and get the `Extract` back.
//!     let value = nextshell::test::request()
//!         .path("/1/2")
//!         .filter(&filter)
//!         .await
//!         .unwrap();
...
//! #[test]
//! fn test_math() {
//! #    let math = || nextshell::any().map(nextshell::reply);
//!     let filter = math();
//!
