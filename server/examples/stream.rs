...
use nextshell::{reply::Response, Filter, Reply};
...
async fn main() {
    ...
    let route = nextshell::body::content_length_limit(65536)
        .and(nextshell::body::stream())
        .then(handler);
    nextshell::serve(route).run(([127, 0, 0, 1], 3030)).await;
}

async fn handler(
    mut body: impl Stream<Item = Result<impl Buf, nextshell::Error>> + Unpin + Send + Sync,
) -> Response {
