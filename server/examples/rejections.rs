...
fn div_by() -> impl Filter<Extract = (NonZeroU16,), Error = Rejection> + Copy {
    nextshell::header::<u16>("div-by").and_then(|n: u16| async move {
...
async fn handle_rejection(err: Rejection) -> Result<impl Reply, Infallible> {
    ...
    if err.is_not_found() {
    ...
    } else if let Some(e) = err.find::<nextshell::filters::body::BodyDeserializeError>() {
        ...
        code = StatusCode::BAD_REQUEST;
    } else if let Some(_) = err.find::<nextshell::reject::MethodNotAllowed>() {
    ...
    let json = nextshell::reply::json(&ErrorMessage {
        code: code.as_u16(),
        message: message.into(),
    });

    Ok(nextshell::reply::with_status(json, code))
}
