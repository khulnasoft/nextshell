...
use nextshell::hyper::StatusCode;
use nextshell::{hyper::Method, reject, Filter, Rejection, Reply};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    nextshell::serve(foo_route.or(bar_route).recover(handle_custom))
        .run(address)
        .await;
