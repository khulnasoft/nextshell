...
async fn main() {
    ...
    // GET /
    let hello_world = nextshell::path::end().map(|| "Hello, World at root!");

    // GET /hi
    let hi = nextshell::path("hi").map(|| "Hello, World!");
    ...
    //
    // GET /sum/:u32/:u32
    let sum = nextshell::path!("sum" / u32 / u32).map(|a, b| format!("{} + {} = {}", a, b, a + b));

    // Any type that implements FromStr can be used, and in any order:
    //
    // GET /:u16/times/:u16
    let times =
        nextshell::path!(u16 / "times" / u16).map(|a, b| format!("{} times {} = {}", a, b, a * b));
    ...
    // GET /math/sum/:u32/:u32
    // GET /math/:u16/times/:u16
    let math = nextshell::path("math").and(sum.or(times));

    // We can use the end() filter to match a shorter path
    let help = nextshell::path("math")
