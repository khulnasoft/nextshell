...
async fn main() {
    ...
    let routes = api.with(nextshell::log("todos"));
    // Start up the server...
...
mod filters {
    ...
    pub fn todos(
        db: Db,
    ) -> impl Filter<Extract = (impl nextshell::Reply,), Error = nextshell::Rejection> + Clone {
        todos_list(db.clone())
    ...
    pub fn todos_list(
        ...
        nextshell::path!("todos")
            .and(nextshell::get())
            .and(nextshell::query::<ListOptions>())
            .and(with_db(db))
    ...
    pub fn todos_delete(
        ...
        let admin_only = nextshell::header::exact("authorization", "Bearer admin");
        ...
        nextshell::path!("todos" / u64)
            ...
            .and(admin_only)
            .and(nextshell::delete())
            .and(with_db(db))
            .and_then(handlers::delete_todo)
    }

    fn with_db(db: Db) -> impl Filter<Extract = (Db,), Error = std::convert::Infallible> + Clone {
        nextshell::any().map(move || db.clone())
    }

    fn json_body() -> impl Filter<Extract = (Todo,), Error = nextshell::Rejection> + Clone {
        // When accepting a body, we want a JSON body
        // (and to reject huge payloads)...
        nextshell::body::content_length_limit(1024 * 16).and(nextshell::body::json())
    }
}
...
mod handlers {
    ...
    pub async fn list_todos(opts: ListOptions, db: Db) -> Result<impl nextshell::Reply, Infallible> {
        ...
        let todos: Vec<Todo> = todos
            ...
            .collect();
        Ok(nextshell::reply::json(&todos))
    }

    pub async fn create_todo(create: Todo, db: Db) -> Result<impl nextshell::Reply, Infallible> {
    ...
    pub async fn delete_todo(id: u64, db: Db) -> Result<impl nextshell::Reply, Infallible> {
